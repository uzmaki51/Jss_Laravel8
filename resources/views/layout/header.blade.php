            ...
            <div class="sp-logout for-sp">
                <li class="dropdown" style="height: auto;">
                    <a href="#" class="dropdown-toggle" data-toggle="dropdown" style="background: transparent;">
                        <img src="{{ Auth::user()->avatar == '' ? ccAsset('assets/avatars/user.png') : Auth::user()->avatar }}" height="24" width="24" style="vertical-align: middle; border-radius: 50%;">
                                            ...
                                            <li>
                                                <a href="/{{ $sub['controller'] == '' ? (count($sub['children']) > 0 ? $sub['children'][0]['controller'] : '') : ($isAdmin && $sub['id'] == 12 ? 'decision/analyzeReport' : $sub['controller']) }}{{ '?menuId=' . $sub['id'] }}">{{ $isAdmin && $sub['id'] == 12 ? '审批分析' : $sub['title'] }}
                                                    @if(count($sub['children']) > 0)
                                                        <img class="has-child" src="{{ ccAsset('assets/img/icons/right-arrow.png') }}">
                                                    @endif
                                                </a>
...
<audio controls="controls" class="d-none" id="warning-audio">
    <source src="{{ ccAsset('assets/sound/delete.wav') }}">
    <embed src="{{ ccAsset('assets/sound/delete.wav') }}" type="audio/wav">
</audio>
<audio controls="controls" class="d-none" id="alert-audio">
    <source allow="autoplay" src="{{ ccAsset('assets/sound/alert.mp3') }}">
    <embed allow="autoplay" src="{{ ccAsset('assets/sound/alert.mp3') }}" type="audio/mp3">
</audio>
<script type="text/javascript">
...
<script src="{{ ccAsset('assets/js/moment.js') }}"></script>
@yield('scripts')
</body>
</html>